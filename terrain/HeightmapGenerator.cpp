...
ns::HeightmapGenerator::HeightmapGenerator(const ns::HeightmapGenerator::Settings& settings)
	:
	settings_(settings),
	data_({ 
		(MapSizeType)settings_.chunkPhysicalSize / (MapSizeType)settings_.numberOfPartitions
		})
{}

std::shared_ptr<ns::HeightmapGenerator::Result> ns::HeightmapGenerator::operator()(const Input& input)
{
	const MapSizeType chunkPosition = settings_.chunkPhysicalSize * (MapSizeType)input;

	//create the array of heights
	std::shared_ptr<ns::HeightmapGenerator::Result> result = std::make_shared<Result>((glm::ivec2)settings_.numberOfPartitions + glm::ivec2(1));
	result->chunk = input;
	...
	for (size_t i = 0; i < result->values.x(); i++)
	{
		...
		{
			result->values.value(i, j) = settings_.generator_(MapSizeType(
	...
	{
		left.positions[i].y = settings_.generator_(MapSizeType(
	...
	for (size_t i = 0; i < HEIGHT; i++)
	{
		right.positions[i].y = settings_.generator_(MapSizeType(
	...
	{
		bottom.positions[i].y = settings_.generator_(MapSizeType(
			settings_.chunkPhysicalSize.x * input.x + i * data_.primitiveSize.x,
	...
	for (size_t i = 0; i < WIDTH; i++)
	{
		top.positions[i].y = settings_.generator_(MapSizeType(
			settings_.chunkPhysicalSize.x * input.x + i * data_.primitiveSize.x,
